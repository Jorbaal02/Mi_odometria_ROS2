...
class ControlOrientacion(Node):
    def __init__(self):
        ...
        self.p_gain = 1.0  # Puedes ajustar este valor
    ...
    def odometry_callback(self, msg):
        ...
        twist.linear.x = 0.0  # Nos aseguramos de que no se mueva linealmente
        self.publisher.publish(twist)

        # Log
        self.get_logger().info(f'Error: {angular_error:.2f}, Velocidad angular: {angular_velocity:.2f}')

    def wrap_angle(self, angle):
        """
        Nos aseguramos de que el ángulo esté entre [-pi, pi]
        """
        while angle > pi:
            angle -= 2 * pi
        while angle < -pi:
